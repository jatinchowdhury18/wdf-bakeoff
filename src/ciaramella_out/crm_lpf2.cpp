...
namespace ciamarella {
    ...
    void lp_filter2::process(float *x, float *y__out__, int nSamples)
    {
        ...
        if (cutoff_CHANGED) {
            static const float R = (1.f / ((lp_filter2__extra__0 * ((0.01f + (0.3f * cutoff)) * fs)) * C));
        ...
        for (int i = 0; i < nSamples; i++) {
            ...
            const float ar_8 = __delayed__6;
            const float ar_9 = ((dl_8 * al_8) + (dr_8 * ar_8));
            const float au_8 = (ar_9 - (lp_filter2__extra__3 * ((al_9 + ar_9) + ((2.f * x[i]) - -((al_9 + ar_9))))));
